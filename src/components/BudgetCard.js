...
export default function BudgetCard({name, amount, max, gray, hideButtons, onAddExpenseClick, onViewExpensesClick, budgetId}) {
  ...
  return (
    <>
      <Card className={classNames.join(" ")}>
          <Card.Body>
              ...
              {!hideButtons && (<Stack direction="horizontal" gap="2" className="mt-4">
                  ...
                  <Button variant="success" onClick={() => setEditModal(true)}><FontAwesomeIcon icon={faPenToSquare} /></Button>
      ...
      <EditModal show={editModal} handleClose={() => setEditModal(false)} name={name} max={max} budgetId={budgetId}/>
    </>
  )
}
